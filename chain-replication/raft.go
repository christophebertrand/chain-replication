...
func newRaftNode(
	...
	rc := &raftNode{
		...
		peers:       peers,
		join:        join,
		waldir:      fmt.Sprintf("persist/raftexample-%d%d", id, clusterID),
		snapdir:     fmt.Sprintf("persist/raftexample-%d%d-snap", id, clusterID),
...
func (rc *raftNode) startRaft() {
	...
	rc.transport = &rafthttp.Transport{
		ID:          types.ID(rc.id),
		ClusterID:   types.ID(rc.clusterID), //0x1000,
		Raft:        rc,
		ServerStats: ss,
...
func (rc *raftNode) serveRaft() {
	url, err := url.Parse(rc.peers[(rc.id-1)%3])
	if err != nil {
